...
create index idx_task_id on train_task_split(task_id);
--更正时间，将每个string 时间都转换成 时间函数
alter table train_task_split add column task_Release_Datetime datetime;
alter table train_task_split add column p_Release_Datetime datetime;
alter table train_task_split add column p_Crowd_Submit_Datetime datetime;
alter table train_task_split add column task_start_datetime datetime;
alter table train_task_split add column task_end_datetime datetime;
alter table train_task_split add column task_days int(11);

update train_task_split set p_Release_Datetime = p_Release_Date;
update train_task_split set task_Release_Datetime=task_Release_Date;
--时间标准化
update train_task_split
	set max_submit_datetime=DATE_FORMAT(max_submit_datetime,'%Y-%m-%d') 
	,min_submit_datetime=DATE_FORMAT(min_submit_datetime,'%Y-%m-%d') ;
--设置起始时间
update train_task_split set task_end_datetime =p_Crowd_Submit_Datetime;
update train_task_split set task_end_datetime = max_submit_datetime
	where max_submit_datetime > p_Crowd_Submit_Datetime;
--设置任务结束时间
update train_task_split set task_start_datetime = p_Release_Datetime;
update train_task_split set task_start_datetime=min_submit_datetime
	where task_start_datetime>min_submit_datetime ; 
--更新任务周期
update train_task_split set task_days=DATEDIFF(task_end_datetime ,task_start_datetime)+1;
...
create table train_base_task
	...
	b.total_amt,
	b.task_Mode
	from train_task_split a
	left join task b on a.task_id = b.task_id
	where a.task_end_datetime>='2015-06-01'
	and a.task_end_datetime<'2015-10-01';
create index idx_task_id on train_base_task(task_id);
alter table train_base_task add column category int(11);
update train_base_task
	set category = case when b.first_catagory='动画漫画'  then 1 
	 				when first_catagory='品牌设计'  then 2 
	 				when first_catagory='摄影摄像'  then 3 
	 				when first_catagory='文案策划'  then 4 
	 				when first_catagory='网站建设'  then 5 
	 				when first_catagory='装修服务'  then 6 
	 				when first_catagory='起名取名'  then 7 
	 				when first_catagory='软件开发'  then 8 end ;

--获取全部稿件信息
create table train_base_piecework as 
...
left join piecework_f b on a.task_id=b.task_id
;
update train_base_piecework a,  piecework_f b
	set a.submit_days=b.submit_days
	where a.piecework_id=b.piecework_id;

create index idx_task_id on train_base_piecework(task_Id);
create index idx_user_id on train_base_piecework(user_id);
create index idx_piecework_id on train_base_piecework(piecework_id);
create index idx_release_user_id on train_base_piecework(release_user_id,user_id);

--user 和 requester
--create table train_ever_meet_in_train 
--	as select distinct release_user_id, user_id
--	from train_base_piecework
--	where task_end_datetime>='2015-06-01'
--	and task_end_datetime<'2015-09-01';
--create table train_ever_meet_in_test 
--	as select distinct release_user_id, user_id
--	from train_base_piecework
--	where task_end_datetime>='2015-09-01'
--	and task_end_datetime<'2015-10-01';
--create index idx_release_user_id on train_ever_meet_in_train(release_user_id,user_id);
--create index idx_release_user_id on train_ever_meet_in_test(release_user_id,user_id);
--create table train_base_ever_met
--	as select distinct a.release_user_id,a.user_id
--	from train_ever_meet_in_test a
--	left join train_ever_meet_in_train b 
--	on a.release_user_id=b.release_user_id and a.user_id=b.user_id
--	where b.release_user_id is not null
--	and b.user_id is not null;
--create index idx_release_user_id on train_base_ever_met(release_user_id,user_id);

--创建 已有用户数据 索引
create table train_base_user_id
	as select distinct user_id  from train_base_piecework b
	where b.task_end_datetime>='2015-06-01' and b.task_end_datetime<'2015-09-01';
create index idx_user_id on train_base_user_id(user_id);

--创建用户数据表
--类目顺序 动画漫画--1;品牌设计--2;摄影摄像--3;文案策划--4;网站建设--5;装修服务--6;起名取名--7;软件开发--8
create table train_worker_summary
	as select a.user_id,
	...
	from train_base_user_id a
	left join train_base_piecework b 
	on a.user_id =b.user_id and b.task_end_datetime>='2015-06-01' and b.task_end_datetime<'2015-09-01'
	group by a.user_id;
alter table train_worker_summary modify column fuwubao int(5);
alter table train_worker_summary modify column recomment_employ int(5);
create index idx_user_id on train_worker_summary(user_id);
create index idx_piecework_id on train_worker_summary(max_piecework_id);
--创建
select user_id,piecework_ct from train_worker_summary where piecework_ct=5 order by piecework_ct desc limit 5;
--创建排序记录
create table train_worker_price_rank as 
...
(	select t1.piecework_id,t1.user_id,t1.total_amt,t1.price_level,@rownum:=@rownum+1,
		...
		@rownum as row_num
		from
	(	(	select piecework_id,user_id,total_amt,price_level from train_base_piecework
				where  task_end_datetime>='2015-06-01' and task_end_datetime<'2015-09-01'
...
create index idx_user_id on train_worker_price_rank(user_id);
alter table train_worker_summary add column median_price int(11);
alter table train_worker_summary add column median_price_level int(2);
--根据rank 更新 median 数据
update train_worker_summary a, train_worker_price_rank b
...
alter table train_worker_summary add column 7d_work_ct_all    int(11);
alter table train_worker_summary add column 7d_work_ct_all_c1 int(11);
alter table train_worker_summary add column 7d_work_ct_all_c2 int(11);
alter table train_worker_summary add column 7d_work_ct_all_c3 int(11);
alter table train_worker_summary add column 7d_work_ct_all_c4 int(11);
alter table train_worker_summary add column 7d_work_ct_all_c5 int(11);
alter table train_worker_summary add column 7d_work_ct_all_c6 int(11);
alter table train_worker_summary add column 7d_work_ct_all_c7 int(11);
alter table train_worker_summary add column 7d_work_ct_all_c8 int(11);
alter table train_worker_summary add column 7d_work_ct_all_p1 int(11);
alter table train_worker_summary add column 7d_work_ct_all_p2 int(11);
alter table train_worker_summary add column 7d_work_ct_all_p3 int(11);
alter table train_worker_summary add column 7d_work_ct_all_p4 int(11);
alter table train_worker_summary add column 7d_work_ct_all_p5 int(11);
alter table train_worker_summary add column 14d_work_ct_all    int(11);
alter table train_worker_summary add column 14d_work_ct_all_c1 int(11);
alter table train_worker_summary add column 14d_work_ct_all_c2 int(11);
alter table train_worker_summary add column 14d_work_ct_all_c3 int(11);
alter table train_worker_summary add column 14d_work_ct_all_c4 int(11);
alter table train_worker_summary add column 14d_work_ct_all_c5 int(11);
alter table train_worker_summary add column 14d_work_ct_all_c6 int(11);
alter table train_worker_summary add column 14d_work_ct_all_c7 int(11);
alter table train_worker_summary add column 14d_work_ct_all_c8 int(11);
alter table train_worker_summary add column 14d_work_ct_all_p1 int(11);
alter table train_worker_summary add column 14d_work_ct_all_p2 int(11);
alter table train_worker_summary add column 14d_work_ct_all_p3 int(11);
alter table train_worker_summary add column 14d_work_ct_all_p4 int(11);
alter table train_worker_summary add column 14d_work_ct_all_p5 int(11);
alter table train_worker_summary add column 30d_work_ct_all    int(11);
alter table train_worker_summary add column 30d_work_ct_all_c1 int(11);
alter table train_worker_summary add column 30d_work_ct_all_c2 int(11);
alter table train_worker_summary add column 30d_work_ct_all_c3 int(11);
alter table train_worker_summary add column 30d_work_ct_all_c4 int(11);
alter table train_worker_summary add column 30d_work_ct_all_c5 int(11);
alter table train_worker_summary add column 30d_work_ct_all_c6 int(11);
alter table train_worker_summary add column 30d_work_ct_all_c7 int(11);
alter table train_worker_summary add column 30d_work_ct_all_c8 int(11);
alter table train_worker_summary add column 30d_work_ct_all_p1 int(11);
alter table train_worker_summary add column 30d_work_ct_all_p2 int(11);
alter table train_worker_summary add column 30d_work_ct_all_p3 int(11);
alter table train_worker_summary add column 30d_work_ct_all_p4 int(11);
alter table train_worker_summary add column 30d_work_ct_all_p5 int(11);

update train_worker_summary a,
    ( select t1.user_id 
	...
	from train_worker_summary t1
	left join train_base_piecework t2 on t1.user_id=t2.user_id and t2.task_end_datetime<'2015-09-01'
	group by t1.user_id
	) b
...
where a.user_id =b.user_id;
--更新质量参数
alter table train_worker_summary add column num_suc_work_ct_all    int(11);
alter table train_worker_summary add column num_suc_work_ct_all_c1 int(11);
alter table train_worker_summary add column num_suc_work_ct_all_c2 int(11);
alter table train_worker_summary add column num_suc_work_ct_all_c3 int(11);
alter table train_worker_summary add column num_suc_work_ct_all_c4 int(11);
alter table train_worker_summary add column num_suc_work_ct_all_c5 int(11);
alter table train_worker_summary add column num_suc_work_ct_all_c6 int(11);
alter table train_worker_summary add column num_suc_work_ct_all_c7 int(11);
alter table train_worker_summary add column num_suc_work_ct_all_c8 int(11);
alter table train_worker_summary add column num_suc_work_ct_all_p1 int(11);
alter table train_worker_summary add column num_suc_work_ct_all_p2 int(11);
alter table train_worker_summary add column num_suc_work_ct_all_p3 int(11);
alter table train_worker_summary add column num_suc_work_ct_all_p4 int(11);
alter table train_worker_summary add column num_suc_work_ct_all_p5 int(11);

alter table train_worker_summary add column num_cost_work_ct_all    int(11);
alter table train_worker_summary add column num_cost_work_ct_all_c1 int(11);
alter table train_worker_summary add column num_cost_work_ct_all_c2 int(11);
alter table train_worker_summary add column num_cost_work_ct_all_c3 int(11);
alter table train_worker_summary add column num_cost_work_ct_all_c4 int(11);
alter table train_worker_summary add column num_cost_work_ct_all_c5 int(11);
alter table train_worker_summary add column num_cost_work_ct_all_c6 int(11);
alter table train_worker_summary add column num_cost_work_ct_all_c7 int(11);
alter table train_worker_summary add column num_cost_work_ct_all_c8 int(11);
alter table train_worker_summary add column num_cost_work_ct_all_p1 int(11);
alter table train_worker_summary add column num_cost_work_ct_all_p2 int(11);
alter table train_worker_summary add column num_cost_work_ct_all_p3 int(11);
alter table train_worker_summary add column num_cost_work_ct_all_p4 int(11);
alter table train_worker_summary add column num_cost_work_ct_all_p5 int(11);

alter table train_worker_summary add column suc_rate_all    decimal(14,4);
alter table train_worker_summary add column suc_rate_all_c1 decimal(14,4);
alter table train_worker_summary add column suc_rate_all_c2 decimal(14,4);
alter table train_worker_summary add column suc_rate_all_c3 decimal(14,4);
alter table train_worker_summary add column suc_rate_all_c4 decimal(14,4);
alter table train_worker_summary add column suc_rate_all_c5 decimal(14,4);
alter table train_worker_summary add column suc_rate_all_c6 decimal(14,4);
alter table train_worker_summary add column suc_rate_all_c7 decimal(14,4);
alter table train_worker_summary add column suc_rate_all_c8 decimal(14,4);
alter table train_worker_summary add column suc_rate_all_p1 decimal(14,4);
alter table train_worker_summary add column suc_rate_all_p2 decimal(14,4);
alter table train_worker_summary add column suc_rate_all_p3 decimal(14,4);
alter table train_worker_summary add column suc_rate_all_p4 decimal(14,4);
alter table train_worker_summary add column suc_rate_all_p5 decimal(14,4);
...
---------------------------piecework部分开始---------------------------------------
create table train_piecework_4train
...
b.work_quality
from  train_base_piecework b ;
--不需要加时间，整体一起算起来，然后等需要用的时候根据时间来进行截取 
--where task_end_datetime>='2015-06-01' and task_end_datetime<'2015-09-01';
create index idx_piecework_id on train_piecework_4train(piecework_id);
create index idx_task_id on train_piecework_4train(task_id);
create index idx_user_id on train_piecework_4train(user_id);
create index idx_release_user_id on train_piecework_4train(release_user_id);
alter table train_piecework_4train modify column relative_reputation_rank int(11);
alter table train_piecework_4train modify column relative_reputation decimal(13,4);
alter table train_piecework_4train modify column submit_rank int(11);
alter table train_piecework_4train modify column relative_submit_rank decimal(13,4);
alter table train_piecework_4train add column task_piece_ct int(11);
--更新rating信息
update train_piecework_4train t1
	left join (	select a.piecework_id,
				...
				from train_piecework_4train a
				left join train_piecework_4train b on a.task_id = b.task_id
...
--更新task信息
alter table train_base_task add column task_piece_ct int(11);
alter table train_base_task add column task_good_piece_ct int(11);
update train_base_task a 
	left join (select task_id,
				...
				from  train_base_piecework t
				group by t.task_id
				)b on a.task_id = b.task_id
set a.task_piece_ct=b.task_piece_ct
,	a.task_good_piece_ct = b.task_good_piece_ct;
update train_piecework_4train a
	left join train_base_task b on a.task_id=b.task_id
	set a.task_piece_ct = b.task_piece_ct;
update train_piecework_4train a
...
--为 piecework 增加 user
create index idx_release_user on train_piecework_4train(release_user_id,user_id);
alter table train_piecework_4train add column ever_met int(2) default 0;
update train_piecework_4train t1,
	 ( select  distinct a.piecework_id
				from train_piecework_4train a
				left join train_piecework_4train b on a.release_user_id=b.release_user_id and a.user_id=b.user_id
...
alter table train_piecework_4train add column selected int(1) default 0;
update train_piecework_4train
	set selected =1
	where work_quality='中标' or work_quality like '%等奖';


--
alter table train_piecework_4train add column task_c1 int(11);
alter table train_piecework_4train add column task_c2 int(11);
alter table train_piecework_4train add column task_c3 int(11);
alter table train_piecework_4train add column task_c4 int(11);
alter table train_piecework_4train add column task_c5 int(11);
alter table train_piecework_4train add column task_c6 int(11);
alter table train_piecework_4train add column task_c7 int(11);
alter table train_piecework_4train add column task_c8 int(11);
alter table train_piecework_4train add column task_p1 int(11);
alter table train_piecework_4train add column task_p2 int(11);
alter table train_piecework_4train add column task_p3 int(11);
alter table train_piecework_4train add column task_p4 int(11);
alter table train_piecework_4train add column task_p5 int(11);

update train_piecework_4train a
	left join train_base_task b on a.task_id=b.task_id
	set  task_c1 = case when b.first_catagory='动画漫画'  then 1 else 0 end
	...
	;
--增加程度表达
alter table train_piecework_4train add column selected_value decimal(5,2);
...
---------------------------piecework部分结束---------------------------------------
---------------------------requester部分开始---------------------------------------
create table train_base_requester_choice as 
	select piecework_id, release_user_id, ability_level_int, 
	 relative_reputation, submit_days
	from train_piecework_4train
	where task_end_datetime>='2015-06-01' and task_end_datetime<'2015-09-01'
		and work_quality ='中标' or work_quality like '%等奖';
alter table train_base_requester_choice add column reputation_median_rank int(11);
alter table train_base_requester_choice add column relative_reputation_median_rank int(11);
alter table train_base_requester_choice add column submit_days_median_rank int(11);
--更新median数据
update train_base_requester_choice a
	left join (	select t1.piecework_id,t1.release_user_id,t1.ability_level_int,@rownum:=@rownum+1,
		...
		@rownum as row_num
		from
	(	(	select piecework_id,release_user_id,ability_level_int from train_base_requester_choice
...
update train_base_requester_choice a
	left join (	select t1.piecework_id,t1.release_user_id,t1.relative_reputation,@rownum:=@rownum+1,
		...
		@rownum as row_num
		from
	(	(	select piecework_id,release_user_id,relative_reputation from train_base_requester_choice
...
update train_base_requester_choice a
	left join (	select t1.piecework_id,t1.release_user_id,t1.submit_days,@rownum:=@rownum+1,
		...
		@rownum as row_num
		from
	(	(	select piecework_id,release_user_id,submit_days from train_base_requester_choice
...
create index idx_release_user_id on train_base_requester_choice(release_user_id);
create index idx_piecework_id on train_base_requester_choice(piecework_id);
--最终的 requester 数据
create table train_base_requester_choice_summary as 
	select release_user_id,
		...
		from train_base_requester_choice group by release_user_id;
alter table train_base_requester_choice_summary modify  column median_selected_reputation           int(11);
alter table train_base_requester_choice_summary modify  column median_selected_relateive_reputation decimal(11,6);
alter table train_base_requester_choice_summary modify  column median_selected_submit_days          int(11);
create index idx_release_user_id on train_base_requester_choice_summary(release_user_id);
update train_base_requester_choice_summary a , train_base_requester_choice b 
...
alter table train_base_requester_choice_summary modify release_user_id int(11);
---------------------------requester部分结束---------------------------------------
...
alter table train_piecework_4train add column work_quality_suc_rate int(3);
alter table train_piecework_4train add column work_quality_suc_rate_cate int(3);
update train_piecework_4train set work_quality_random =0;
update train_piecework_4train set work_quality_higher =0;	
update train_piecework_4train set work_quality_suc_rate =0;	
update train_piecework_4train set work_quality_suc_rate_cate =0;	
...
update train_piecework_4train a,
	 (	select t1.piecework_id,t1.task_id,@rownum:=@rownum+1,
			...
			@rownum as row_num
			from
		(	(	select m1.piecework_id,m1.task_id from train_piecework_4train m1
			left join train_worker_summary m2 on m1.user_id=m2.user_id
...
update train_piecework_4train a,
	 (	select t1.piecework_id,t1.task_id,t1.ability_level_int,@rownum:=@rownum+1,
			...
			@rownum as row_num
			from
		(	(	select piecework_id,task_id,ability_level_int from train_piecework_4train
...
update train_piecework_4train a,
	 (	select t1.piecework_id,t1.task_id,t1.suc_rate_all,@rownum:=@rownum+1,
			...
			@rownum as row_num
			from
		(	(	select m1.piecework_id,m1.task_id,m2.suc_rate_all from train_piecework_4train m1
			left join train_worker_summary m2 on m1.user_id=m2.user_id
...
update train_piecework_4train a,
	 (	select t1.piecework_id,t1.task_id,t1.cate_suc_rate,@rownum:=@rownum+1,
		...
		(	(	select m1.piecework_id,m1.task_id,
			...
			from train_piecework_4train m1
			left join train_worker_summary m2 on m1.user_id=m2.user_id
			left join train_base_task m3 on m1.task_id=m3.task_id
...
---------------------------------------------------------------------------------------
-----------------------------pairwise--------------------------------------------------
select count(1) from train_piecework_4train a
left join train_piecework_4train b
on a.task_id = b.task_id
where a.task_end_datetime<"2015-09-01"
and a.selected_value<>b.selected_value
;

create table train_piecework_4train_order1
as 
select @rank:=@rank+1 id, t1.* from 
(select a.* from train_piecework_4train a
left join train_worker_zero b on a.user_id = b.user_id
where a.task_end_datetime<"2015-09-01"
...
;
create index idx_task_id on train_piecework_4train_order1(task_id);
create table train_piecework_4train_order2
as 
select @rank:=@rank+1 id, t1.* from 
(select a.* from train_piecework_4train a
where a.task_end_datetime>="2015-09-01"
...
order by t1.task_id asc, t1.piecework_id asc
;
create index idx_task_id on train_piecework_4train_order2(task_id);


create table train_piecework_pairwise_1
select 
...
from train_piecework_4train_order1 a
left join train_piecework_4train_order1 b
on a.task_id = b.task_id
where a.selected_value<>b.selected_value
;

create table train_piecework_pairwise_2
select 
cast(a.piecework_id as decimal(22,0)) as a_pid, 
cast(b.piecework_id as decimal(22,0)) as b_pid,
a.id as a_id,
...
from train_piecework_4train_order2 a
left join train_piecework_4train_order2 b
on a.task_id = b.task_id
;


create index idx_task_id on train_piecework_pairwise_2(task_id);
update train_piecework_pairwise_2 a force index (idx_task_id) left join train_base_task b force index (idx_task_id) on a.task_id =b.task_id set a.category = b.category;


update train_piecework_pairwise_1 a force index (idx_task_id) left join train_base_task b force index (idx_task_id) on a.task_id =b.task_id set a.category = b.category;


explain update train_piecework_pairwise_2 a  left join train_base_task b  on a.task_id =b.task_id set a.category = b.category;

explain update train_piecework_pairwise_2 a  left join train_base_task b  on a.idx_task =b.task_id set a.category = b.category;

alter table train_piecework_pairwise_1 add column idx_task varchar(64);
update train_piecework_pairwise_1 set idx_task=task_id;
create index idx_task on train_piecework_pairwise_1(idx_task);
update train_piecework_pairwise_1 a  left join train_base_task b  on a.idx_task =b.task_id set a.category = b.category;
